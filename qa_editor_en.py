...
def _gemini_factcheck_en(body_text):
    """
    Backup Fact-Check: Uses Gemini 2.0 Flash + Search Grounding tools.
    Triggers only if NotebookLM fails or returns no alerts.
    """
    try:
...
\n{body_text[:8000]}\n
Output ONLY a bulleted list of 3 specific alerts in English. If everything is correct, return an empty string."""
        
        res = researcher._layer_2_gemini_grounding(topic, brief, lang="en")
        if res and "content" in res:
            content = res["content"]
            if len(content) > 50:
                return f"VERIFICATION ALERTS (GEMINI GROUNDING BACKUP):\n{content}"
    except Exception as e:
        print(f"   ⚠️ Error in Gemini Backup Fact-check: {e}")
    return ""
...
def _force_inject_links(body_text, sources, lang="en", category="general"):
    """
    Surgical Link Deposit Injection: Instead of processing the entire article, 
    extracts a couple of long paragraphs and asks the LLM to rewrite them injecting the link.
    """
    import math
    if not sources:
        return body_text
        
    print(f"      🔗 Forcing surgical injection of {len(sources)} authorized sources via LLM...")
    
    # Split into paragraphs
    paragraphs = body_text.split('\n\n')
    
    # Find valid paragraphs for injection 
    # (minimum 150 characters, not titles/lists/tables/quotes)
    valid_indices = []
    ...
    if not valid_indices:
        print("      ⚠️ No valid paragraphs found for injection. Aborting.")
        return body_text
        
    # Limit injection to max 3 sources
    sources_to_inject = sources[:3]
    
    # Choose evenly distributed indices
    chosen_indices = []
    ...
    for i, p_idx in enumerate(chosen_indices):
        ...
        prompt = f"Rewrite the following paragraph in English to naturally inject this link: {url}\nUse a descriptive and very fluent anchor text that perfectly fits the context of the sentence natively. Return ONLY the rewritten paragraph, no quotes, no code blocks, and no extra text.\n\nPARAGRAPH:\n{original_p}"
        
        result = _call_llm_en(prompt, sys_prompt)
        
        # Basic validation: must return something, with the link, and not artificially inflate length.
    ...
    if successful_injections > 0:
        print(f"      ✅ Surgical injection completed: {successful_injections} links inserted.")
        return '\n\n'.join(paragraphs)
    else:
        print("      ⚠️ LLM injection failed. Keeping original.")
        return body_text
...
def run(category, content_dir="content/en"):
    ...
    # STEP 1.7: INYECCIÓN FORZADA DE ENLACES EXTERNOS (LINK DEPOSIT)
    has_ext_link = bool(re.search(r'\]\(https?://[^\)]+\)', body))
    if not has_ext_link and article_slug:
        print(f"\n   🔌 [Editor EN] STEP 1.7: Link Deposit (Forced Injection) - 0 Links detected...")
        sources_to_inject = []
        try:
            # DECOUPLED: Use only the file for this language
            with open("data/source_links_en.json", "r", encoding="utf-8") as f:
                deposit = json.load(f)
        ...
        if sources_to_inject:
            body = _force_inject_links(body, sources_to_inject, lang="en", category=category)
        else:
            print(f"      ⚠️ No source links deposited for '{article_slug}'.")

    # STEP 2: Hybrid Fact-Check (NotebookLM with Gemini Fallback)
    print(f"\n   🔍 [Editor EN] STEP 2: Fact-check with NotebookLM...")
    factcheck_alerts = _notebooklm_factcheck_en(body)

    if not factcheck_alerts:
        print(f"\n   🥈 [Editor EN] NotebookLM failed or returned no alerts. Using Gemini Search Grounding as Backup...\n")
        factcheck_alerts = _gemini_factcheck_en(body)
