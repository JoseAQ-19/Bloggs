...
from niche_registry import NICHES, STRUCTURE_TEMPLATES, get_silo, get_author
...
def planificar_articulo(tema, contexto, lang, category_config):
    ...
    def fallback_plan(p, s):
        resp = client.models.generate_content(model='gemini-2.5-flash', contents=p, config=types.GenerateContentConfig(response_mime_type="application/json"))
        return resp.text.strip()
...
def _call_nvidia_nim(prompt_text, model_id, calibration_tag, nvidia_key, max_tokens=8192, force_json=False):
    ...
    try:
        ...
        if force_json:
            ...
            system_msg += (
                "\n\n[JSON MODE]: Debes devolver ÚNICAMENTE un objeto JSON válido con la "
                "estructura solicitada. Sin formato markdown, sin texto adicional, sin ```json. "
                "Solo el JSON puro."
            )

        # Alinear con la guía oficial de NIM: permitir razonamiento interno
...
def _call_en_engine_v3_core(prompt_text, system_prompt=""):
    ...
    resp = client.models.generate_content(model='gemini-2.5-flash', contents=prompt_text + CAL_GEMINI)
    return resp.text.strip()
...
def _call_es_engine_v3_core(prompt_text, system_prompt=""):
    ...
    resp = client.models.generate_content(model='gemini-2.5-flash', contents=prompt_text + CAL_GEMINI_ES)
    return resp.text.strip()
...
🌐 GEO-DOMINANCE & FORMATTING (GEO/AEO 2026):
- The 3 TL;DR bullets (described above) ARE the GEO signal. No separate "Key Takeaways" header needed.
- RESPONSE BLOCK CHUNKING (40-60 WORDS): The first paragraph directly following EVERY H2 and H3 header MUST be between 40 and 60 words, providing a direct, synthetic, and definitive answer to the query with key terms in **bold**.
- MANDATORY MARKDOWN TABLE: You MUST include at least ONE clean, responsive Markdown table (| Metric | Spec | Value |) with numerical, comparative, or technical data to maximize Information Gain.
- MANDATORY FAQ SECTION: Include a section `## Preguntas Frecuentes` (ES) or `## Frequently Asked Questions` (EN) before closing, featuring 3 to 5 Q&A items using `### [Clear Question]` headers with 40-60 word synthetic responses.
...
def extract_faq_items(contenido):
    """
    Extrae pares de pregunta-respuesta de la sección '## Preguntas Frecuentes' o '## Frequently Asked Questions'
    para inyectar en el Frontmatter YAML y alimentar la estructura JSON-LD FAQPage.
    """
    faq_list = []
    if not contenido:
        return faq_list
    
    faq_match = re.search(r'(?:^|\n)\s*##\s*(?:Preguntas Frecuentes|Frequently Asked Questions)(.*?)(?:\n\s*##[^\#]|\Z)', contenido, re.DOTALL | re.IGNORECASE)
    if not faq_match:
        return faq_list
    ...
    for chunk in chunks:
        ...
        if question and answer:
            faq_list.append({"question": question, "answer": answer[:350]})
    return faq_list


def guardar_post(meta, contenido, lang, category, forced_image=None, translation_key=None, contexto=None):
    """Guarda el post en estructura Leaf Bundle de Hugo (content/{lang}/{category}/{slug}/index.md) con imagen optimizada y caché de investigación."""
    # 💎 SAFETY NET: Purge any metadata leaks before processing
    contenido = ContentCleaner.sanitize_body(contenido)
    
    # Categorización en Silos Temáticos Hierárquicos & Autor Experto
    silo_category = get_silo(category)
    author_name = get_author(category, lang)
    faq_items = extract_faq_items(contenido)
    
    # Lógica Leaf Bundle: content/{lang}/{category}/{slug}/
    bundle_dir = f"content/{lang}/{category}/{meta['slug']}"
    os.makedirs(bundle_dir, exist_ok=True)
    filepath = f"{bundle_dir}/index.md"
    
    # Guardar caché de investigación research_brief.json en la carpeta del bundle
    if contexto is not None:
        try:
            brief_file = f"{bundle_dir}/research_brief.json"
            brief_content = contexto if isinstance(contexto, dict) else {"context": str(contexto)}
            with open(brief_file, "w", encoding="utf-8") as bf:
                json.dump(brief_content, bf, ensure_ascii=False, indent=2)
            print(f"   💾 Cache de investigación guardado: {brief_file}")
        except Exception as e:
            print(f"   ⚠️ Error guardando research_brief.json: {e}")

    # Lógica de Imagen: Generar u obtener guardando en la carpeta del bundle
    width, height = 1200, 675
    if forced_image:
        imagen = forced_image
    else:
        img_res = get_image(meta['titulo'], contenido, meta['slug'], category, bundle_dir=bundle_dir)
        if isinstance(img_res, tuple):
            imagen, width, height = img_res
        else:
            imagen = img_res
    
    # VALIDACIÓN BLINDADA: Si la imagen es una URL externa o vacía, usar fallback local
    if not imagen or not isinstance(imagen, str) or imagen.startswith("http"):
        print(f"   ⚠️ Imagen inválida detectada: '{imagen}'. Usando fallback local...")
        imagen = "featured.webp"
    ...
    abs_image = f"https://novumworld.com/{lang}/{category}/{meta['slug']}/{imagen}" if not imagen.startswith('http') else imagen
...
<div class="author-box" style="padding: 20px; background-color: #f8f9fa; border-left: 4px solid #0056b3; margin-top: 30px; border-radius: 4px;">
    {("<h4>✍️ Sobre el Analista</h4><p><strong>" + author_name + "</strong> es un especialista en el área dedicado a decodificar tendencias institucionales y flujos de capital. Nuestros reportes cruzan datos técnicos y macroeconomía para ofrecer proyecciones libres de ruido corporativo.</p>" if lang == 'es' else "<h4>✍️ About the Analyst</h4><p><strong>" + author_name + "</strong> is a domain specialist dedicated to decoding institutional trends and capital flows. Our reports cross-reference technical data and macroeconomics to deliver noise-free projections.</p>")}
</div>
...
""" if category in ['crypto', 'funds', 'stocks'] else ""
    ...
    safe_meta = {
        ...
        "author": author_name,
        "tags": [niche_name],
        "categories": [silo_category, category],
        "type": category,
        "language": lang,
        "translationKey": translation_key
    }
    if faq_items:
        safe_meta["faq"] = faq_items
...
def guardar_fuentes(slug, sources, lang="es"):
    """
    ...
    try:
        os.makedirs("data", exist_ok=True)
        # DECOUPLED: Usar fichero por idioma para evitar condiciones de carrera en Git
        ...
        print(f"   ⚠️ [Link Deposit] Error guardando fuentes ({lang}): {e}")
