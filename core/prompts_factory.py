...
### REGLAS DE FORMATO CRÍTICAS (PARA APROBACIÓN DE ADSENSE & GEO/AEO 2026):
...
0. ZERO METADATA LEAK & NO H1 MANDATE:
   - **ESTRICTAMENTE PROHIBIDO** imprimir el título del artículo (H1) en tu respuesta. El framework lo pondrá por ti.
...
1. REGLA DEL BLOQUE DE RESPUESTA (CHUNKING RAG / AEO 2026):
   - El primer párrafo inmediatamente posterior a CADA encabezado (H2 y H3) DEBE tener entre 40 y 60 palabras.
   - Debe responder de forma sintética, directa y definitiva a la consulta del encabezado para permitir la extracción autónoma por motores de búsqueda de IA (Google AI Overviews / Perplexity).
   - Incluye la entidad o cifra clave en **negrita** dentro de este párrafo sintético.

2. TABLA MARKDOWN OBLIGATORIA:
   - OBLIGATORIO incluir al menos UNA tabla Markdown bien estructurada (`| Encabezado 1 | Encabezado 2 |`) con datos numéricos, comparativas o métricas concretas. Las tablas deben ser claras y legibles en dispositivos móviles.

3. SECCIÓN FINAL DE PREGUNTAS FRECUENTES (FAQ):
   - OBLIGATORIO incluir antes del cierre del artículo una sección con el título exacto `## Preguntas Frecuentes`.
   - Debe contener entre 3 y 5 preguntas en subencabezados `### [Pregunta en formato claro]`.
   - Cada respuesta debe ser un bloque directo y claro de entre 40 y 60 palabras.

4. EL ESTÁNDAR "OBRA MAESTRA" (E-E-A-T & LOCALIZACIÓN):
   - OBLIGATORIO empezar el artículo DIRECTAMENTE con este encabezado EXACTO (No uses 'Key Insights'):
     ## Resumen Ejecutivo
     - [Punto clave 1...]
     - [Punto clave 2...]
   - ESTRICTAMENTE PROHIBIDO: generar apartados de "Metodología", "Fuentes", "Metodología y Fuentes", "Referencias", "Artículos relacionados", "Artículos Relacionados", "Related Articles", "Sources", "Methodology and Sources", ni ningún bloque de "Lecturas Recomendadas" o "Disclaimers/Descargos de responsabilidad". El texto debe terminar de forma natural tras las Preguntas Frecuentes. El sistema de post-procesamiento ensamblará TODO el pie de página.

5. UX / LEGAL (DISCLAIMER YMYL ÚNICO):
...
6. SINTAXIS "ANTI-GPT" (PROHIBIDO):
   - Sin preguntas retóricas en el cuerpo del texto (excepto en los encabezados `###` de la sección `## Preguntas Frecuentes`).
   - Sin clichés de IA: "solo el tiempo dirá", "en conclusión", "arma de doble filo", "en resumen".
   - PROHIBIDO usar encabezados como "Introducción", "Conclusión", "Resumen".

7. ENLACES: Al menos 3 enlaces HTTPS naturales en el texto.
8. JERARQUÍA: Solo usa ## y ###. No uses #.
9. LONGITUD: 2500 palabras.
"""

SYSTEM_FORMAT_RULES_EN = """
### CRITICAL FORMAT RULES (FOR ADSENSE APPROVAL & GEO/AEO 2026):
...
1. RESPONSE BLOCK RULE (CHUNKING RAG / AEO 2026):
   - The first paragraph immediately following EVERY header (H2 and H3) MUST be between 40 and 60 words.
   - It MUST provide a direct, synthetic, and definitive answer to the header's query for AI Search Engines (Google AI Overviews / Perplexity).
   - Bold key entities or data points (**bold**) within this synthetic block.

2. MANDATORY MARKDOWN TABLE:
   - OBLIGATORY to include at least ONE well-formatted Markdown table (`| Column 1 | Column 2 |`) featuring numerical data, performance metrics, or comparisons to increase Information Gain.

3. MANDATORY FREQUENTLY ASKED QUESTIONS SECTION (FAQ):
   - OBLIGATORY to include a final section with the exact title `## Frequently Asked Questions`.
   - Must contain 3 to 5 questions under `### [Clear Question]` headers.
   - Each answer must be a direct 40–60 word paragraph.

4. THE "MASTERPIECE" STANDARD (E-E-A-T & LOCALIZATION):
   - OBLIGATORY to start the article DIRECTLY with this EXACT header (Do NOT use 'Key Insights'):
     ## Executive Summary
     - [Key point 1...]
     - [Key point 2...]
   - STRICTLY FORBIDDEN: Do NOT generate any sections called "Methodology", "Sources", "Methodology and Sources", "Metodología y Fuentes", "Related Articles", "Artículos relacionados", "References", "Recommended Reading" or any kind of "Disclaimer" / "Disclosure". The text must end naturally with the FAQ section. The post-processing system will assemble the ENTIRE footer.

5. UX / LEGAL (UNIQUE YMYL DISCLAIMER):
...
6. "ANTI-GPT" SYNTAX SHIELD (FORBIDDEN):
   - NO Rhetorical Questions in the body (except for `###` headers in the `## Frequently Asked Questions` section).
   - NO AI Clichés: "only time will tell", "in conclusion", "double-edged sword", "in summary".
   - FORBIDDEN to use headers like "Introduction", "Conclusion", "Summary".

7. HYPERLINKS: At least 3 natural HTTPS links embedded in the text.
8. HIERARCHY: Use ## and ### only. Do NOT use #.
9. LENGTH: 2500 words.
"""
